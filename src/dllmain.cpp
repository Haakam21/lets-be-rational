...
namespace {
  ...
  bool HaveExcel() {
    switch (XLOper::APIMode) {
      case XLOper::APIMode4: [[fallthrough]];
      case XLOper::APIMode12: return true;
      default: break;
    }
    return false;
  }
...
# endif
  ...
  const std::string& file_name = this_file_name(), & xll_name = this_xll_name(), & xl_category_name = this_xl_category_name();
...
# endif
  ...
  for (size_t i = 0; i < funcs.size(); ++i) {
    XLOper dll_name(funcs[i].DLLName), data_codes(XLOper::replace_xloper_codes(funcs[i].DataCodes.c_str())), xl_name('.' == funcs[i].XLName[0] ? xl_category_name + funcs[i].XLName : funcs[i].XLName), arg_names(funcs[i].ArgumentNames), func_desc(replace(funcs[i].FunctionDescription, "@THIS_XLL_NAME@", xll_name));
...
#  if defined(VERBOSE_DEBUG_LOG)
      ...
      if (XLOper::APIModeUndefined == XLOper::APIMode) DebugPrintf("Found no Excel callback interface.\n");
